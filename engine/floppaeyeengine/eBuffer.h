...
#define	CB_VERTEX (UINT)1
#define	CB_CONSTANT (UINT)2
#define	CB_INDEX (UINT)3
...
public:
	...
	bool CreateBuffer(ID3D11Device* pDevice, void* pBufferData, UINT sizeOfBuffer, UINT Type) {
		...
		if (Type == CB_VERTEX) { fBind = D3D11_BIND_VERTEX_BUFFER; }
		if (Type == CB_INDEX) { fBind = D3D11_BIND_INDEX_BUFFER; }
		if (Type == CB_CONSTANT) { fBind = D3D11_BIND_CONSTANT_BUFFER; }
